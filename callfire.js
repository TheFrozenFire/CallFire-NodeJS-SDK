...
(function() {
    ...
    proto.parse_resource_node = function(element) {
        ...
        if(this.queryMap[resource_name] !== undefined) {
            resource = new this.resource[resource_name]();
    ...
    proto.subscription = function(body) {
        var document = libxmljs.parseXml(body);
        ...
        var subscription_object = null;
        
        if(root !== undefined) {
            switch(root.name()) {
                case 'CallFinished':
                    subscription_object = new this.subscription.CallFinished(root);
                    break;
                case 'TextNotification':
                    subscription_object = new this.subscription.TextNotification(root);
                    break;
            }
        }
    ...
    proto.subscription.CallFinished = require('./Subscription/CallFinished');
    proto.subscription.TextNotification = require('./Subscription/TextNotification');
